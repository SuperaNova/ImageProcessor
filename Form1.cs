...
namespace ImageProcessor
{
    public partial class Form1 : Form
    {
        ...
        private bool UseConvolution => chkUseConvolution != null && chkUseConvolution.Checked && cmbConvolutionType.SelectedIndex > 0;
private ConvMatrix GetSelectedConvMatrix()
{
    switch (cmbConvolutionType.SelectedItem?.ToString())
    {
        case "Smooth": return ConvMatrix.Smooth(2);
        case "Gaussian Blur": return ConvMatrix.GaussianBlur();
        case "Sharpen": return ConvMatrix.Sharpen();
        case "Edge Detect": return ConvMatrix.EdgeDetect();
        default: return null;
    }
}

private void ApplyConvolutionIfEnabled(ref Bitmap bmp)
{
    if (UseConvolution)
    {
        var matrix = GetSelectedConvMatrix();
        if (matrix != null)
        {
            ConvMatrix.Conv3x3(bmp, matrix);
        }
    }
}
...
private void btnGreyscale_Click(object sender, EventArgs e)
{
    if (processedImage != null)
    {
        ...
        ApplyConvolutionIfEnabled(ref bmp);
